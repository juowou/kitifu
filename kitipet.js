...
const updateDateNumeric = (e) => e.getFullYear() * 1000000 + (e.getMonth() + 1) * 10000 + e.getDate() * 100 + e.getHours(); 
  ...
  const getHungry = () => {
    ...
    hungry.timer = hungry.timer.filter(time => {
      if (time !== '' && time <= currentDate) {
          console.log(hungry.level + '-1 getHungry');
          hungry.level -= 1;
          hungryLevelDisplay(hungry.level);
          localStorage.setItem('hungry', JSON.stringify(hungry));
          return false; // Remove this time
      }
      return true; // Keep this time
  });
  ...
  const getPoop = () => {
    ...
    currentDate = updateDateNumeric(newDate);
    poop.timer = poop.timer.filter(time => {
      if (time !== '' && time <= currentDate) {
  ...
  const getHate = () => {
    ...
    currentDate = updateDateNumeric(newDate);
    love.timer = love.timer.filter(time => {
      if (time !== '' && time <= currentDate) {
          console.log(love.level + '-1 getHungry');
          love.level -= 1;
          loveLevelDisplay(love.level);
          localStorage.setItem('love', JSON.stringify(love));
          return false; // Remove this time
